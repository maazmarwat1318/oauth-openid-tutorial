...
export class HomeComponent {
  ...
  async onAuth() {
    ...
    let authTokenRequestUrl = encodeURI(
      `${environment.oAuthUrl}?scope=https://www.googleapis.com/auth/gmail.readonly openid email profile&prompt=consent&redirect_uri=http://localhost:4200/oauth&response_type=code&client_id=${environment.oAuthClientId}&code_challenge=${codeChallenge}&code_challenge_method=S256`
    );
    ...
    window.addEventListener('message', async (event) => {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        this._tokenService.setIdToken(data.id_token);
